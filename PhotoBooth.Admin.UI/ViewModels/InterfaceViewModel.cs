...
namespace PhotoBooth.Admin.UI.ViewModels
{
    public sealed class InterfaceViewModel : PageViewModel
    {
        ...
        async Task Import() { var path = dialog.PickBackground(); if (path == null) return; try { SelectedAsset = await assets.ImportAsync(path, CancellationToken.None); Message = "Đã nhập ảnh nền."; await Load(); SelectedAsset = Find(SelectedAsset?.Id); } catch (Exception e) { Fail(e, e.Message); } }
        async Task Select() { try { var id = SelectedAsset.Id; await assets.SelectAsync(id, CancellationToken.None); Message = "Đã chọn ảnh nền cho màn hình khách."; await Load(); SelectedAsset = Find(id); } catch (Exception e) { Fail(e, "Không thể chọn ảnh nền"); } }
        ...
        async Task ResetLiveView() { ShowLiveView = true; LiveViewX = 50; LiveViewY = 50; LiveViewAreaPercent = 5; BackgroundZoom = 100; BackgroundPanX = 0; BackgroundPanY = 0; await SaveLiveView(); }
