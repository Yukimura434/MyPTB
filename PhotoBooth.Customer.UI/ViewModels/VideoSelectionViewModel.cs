...
namespace PhotoBooth.Customer.UI.ViewModels
{
    public sealed class VideoSelectionViewModel : ObservableObject
    {
            ...
            IPrinterService printers, IPrintPipeline printPipeline, ILogger<VideoSelectionViewModel> log)
        {
            ...
            machine.StateChanged+=(s,e)=>{if(machine.State==CustomerWorkflowState.VideoSelection)_=Load();};
            if(machine.State==CustomerWorkflowState.VideoSelection)_=Load();
        }
        ...
        async Task Load()
        {
            try
            {
                ErrorMessage=null;FrameSlots.Clear();Videos.Clear();PrintCopies=1;
                ...
                for(var i=0;i<shots.Count;i++){var video=await videos.CreatePreviewVideoAsync(shots[i].VideoPath,previewDirectory,CancellationToken.None);Videos.Add(new CapturedPhotoChoice(shots[i].VideoPath,i+1,video,shots[i].PicturePath));}
            ...
            }catch(Exception e){Fail(e,"Không thể tải video");}
        }
        ...
        async Task Preview()
        {
            ...
            var next=await composer.ComposeAsync(working,FrameWithTransforms(),context.DefaultPreset,false,map,CancellationToken.None);
        ...
        async Task Finish()
        {
            try
            {
                ...
                var rawStill=await composer.ComposeAsync(rawWorking,transformedFrame,null,false,pictureAssignments,CancellationToken.None);
                try{await videos.ComposeAsync(rawStill,transformedFrame,assignments,destination,CancellationToken.None);}
                ...
                if(string.IsNullOrWhiteSpace(context.CaptureId))
                {
                    ...
                    var capture=await captures.CreateWithCompositeVideoAsync(context.Session.Id,SelectedFrame.Id,context.Session.FinalImageId,still,context.CurrentShots,destination,assignments.Values.ToList(),expires,CancellationToken.None);context.CaptureId=capture.Id;
                }
