...
namespace PhotoBooth.Infrastructure.Services
{
    ...
    public sealed class StorageManager : IStorageManager
    {
        ...
        readonly ApplicationOptions options; readonly ISettingsService settings; readonly ISessionRepository sessions; readonly IMediaAssetRepository assets;
        readonly string root; readonly string rootPrefix;
        public StorageManager(ApplicationOptions options,ISettingsService settings,ISessionRepository sessions,IMediaAssetRepository assets){this.options=options;this.settings=settings;this.sessions=sessions;this.assets=assets;root=Path.GetFullPath(options.DataDirectory).TrimEnd(Path.DirectorySeparatorChar,Path.AltDirectorySeparatorChar);rootPrefix=root+Path.DirectorySeparatorChar;foreach(var area in Areas)Directory.CreateDirectory(GetPath(area));}
        ...
        public SessionStoragePaths CreateSessionStorage(Guid sessionId,DateTime startedAtUtc){if(sessionId==Guid.Empty)throw new ArgumentException("Session ID is required.",nameof(sessionId));var utc=startedAtUtc==default(DateTime)?DateTime.UtcNow:startedAtUtc.ToUniversalTime();var sessionRoot=Path.Combine(GetPath("Captures"),utc.ToString("yyyy"),utc.ToString("MM"),utc.ToString("dd"),sessionId.ToString("N"));var value=new SessionStoragePaths{Root=sessionRoot,Work=Path.Combine(sessionRoot,"Work"),Originals=Path.Combine(sessionRoot,"Originals"),Final=Path.Combine(sessionRoot,"Final")};Directory.CreateDirectory(value.Work);Directory.CreateDirectory(value.Originals);Directory.CreateDirectory(value.Final);return value;}
        public string GetRelativePath(string fullPath){if(string.IsNullOrWhiteSpace(fullPath))throw new ArgumentException("Path is required.",nameof(fullPath));var path=Path.GetFullPath(fullPath);if(!path.StartsWith(rootPrefix,StringComparison.OrdinalIgnoreCase))throw new InvalidOperationException("Media path is outside the managed data root.");return path.Substring(rootPrefix.Length).Replace(Path.DirectorySeparatorChar,'/');}
        public string GetFullPath(string relativePath){if(string.IsNullOrWhiteSpace(relativePath)||Path.IsPathRooted(relativePath))throw new InvalidOperationException("Managed media path must be relative.");var path=Path.GetFullPath(Path.Combine(root,relativePath.Replace('/',Path.DirectorySeparatorChar)));if(!path.StartsWith(rootPrefix,StringComparison.OrdinalIgnoreCase))throw new InvalidOperationException("Media path escapes the managed data root.");return path;}
        public async Task CleanupAsync(CancellationToken token){var s=await settings.GetAsync(token)??new Settings();CleanupFiles(GetPath("Temp"),DateTime.UtcNow.AddHours(-Math.Max(1,s.TemporaryFileRetentionHours)),token);CleanupFiles(GetPath("Preview"),DateTime.UtcNow.AddHours(-Math.Max(1,s.TemporaryFileRetentionHours)),token);await CleanupManagedSessions(DateTime.UtcNow.AddDays(-Math.Max(1,s.SessionRetentionDays)),token).ConfigureAwait(false);}
        async Task CleanupManagedSessions(DateTime cutoff,CancellationToken token){var captures=GetPath("Captures");if(!Directory.Exists(captures))return;foreach(var path in Directory.EnumerateDirectories(captures,"*",SearchOption.AllDirectories).OrderByDescending(x=>x.Length)){token.ThrowIfCancellationRequested();Guid id;if(!Guid.TryParseExact(Path.GetFileName(path),"N",out id))continue;var session=await sessions.GetAsync(id,token).ConfigureAwait(false);if(session==null||!session.IsBoothSession||session.CompletedAtUtc==null||session.CompletedAtUtc.Value>=cutoff||string.IsNullOrWhiteSpace(session.OutputDirectory))continue;var registered=Path.GetFullPath(session.OutputDirectory).TrimEnd(Path.DirectorySeparatorChar,Path.AltDirectorySeparatorChar);var candidate=Path.GetFullPath(path).TrimEnd(Path.DirectorySeparatorChar,Path.AltDirectorySeparatorChar);if(!string.Equals(registered,candidate,StringComparison.OrdinalIgnoreCase))continue;if(await assets.HasPendingOutputAsync(id,token).ConfigureAwait(false))continue;try{Directory.Delete(path,true);await assets.MarkDeletedBySessionAsync(id,token).ConfigureAwait(false);}catch(IOException){}catch(UnauthorizedAccessException){}}}
        static void CleanupFiles(string root,DateTime cutoff,CancellationToken token){foreach(var file in Directory.EnumerateFiles(root,"*",SearchOption.AllDirectories)){token.ThrowIfCancellationRequested();try{if(File.GetLastWriteTimeUtc(file)<cutoff)File.Delete(file);}catch{}}foreach(var dir in Directory.EnumerateDirectories(root,"*",SearchOption.AllDirectories).OrderByDescending(x=>x.Length))try{if(!Directory.EnumerateFileSystemEntries(dir).Any())Directory.Delete(dir);}catch{}}
    }
