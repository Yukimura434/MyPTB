...
namespace PhotoBooth.Database
{
 public sealed class SqliteStatsRepository:IStatsRepository
 {
  ...
  public Task<long> CountCapturedImagesAsync(CancellationToken t)=>Task.FromResult(Count("SELECT COUNT(*) FROM CapturePhotos WHERE PhotoType IN ('Picture','MotionPhoto','Original')"));
