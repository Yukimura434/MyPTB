...
{
    public class DeliverableIntegrityService : ICaptureIntegrityService, IDeliverableIntegrityService
    {
        public Task ValidateAsync(Deliverable deliverable, CancellationToken token) => ValidateAsync((PhotoCapture)deliverable, token);
        public Task ValidateAsync(PhotoCapture capture, CancellationToken token)
        {
            if (capture == null || string.IsNullOrWhiteSpace(capture.Id)) throw new InvalidDataException("Deliverable identity is unavailable.");
            ...
            if (assets.Count == 0 || assets.Any(x => string.IsNullOrWhiteSpace(x.Id) || !string.Equals(x.CaptureId, capture.Id, StringComparison.Ordinal))) throw new InvalidDataException("Deliverable asset ownership is incomplete.");
            ...
            if (pictures.Count == 0 || composites.Count != 1) throw new InvalidDataException("Deliverable must contain original Pictures and one static Composite.");
            ...
            {
                if (pictures.Count != videos.Count || compositeVideos.Count != 1) throw new InvalidDataException("PictureAndVideo deliverable has an incomplete asset pair.");
            ...
            else if (videos.Count != 0 || compositeVideos.Count != 0) throw new InvalidDataException("PictureOnly deliverable contains unexpected video assets.");
        ...
        static void ValidateFile(CapturePhoto asset, CancellationToken token)
        {
            ...
            if (string.IsNullOrWhiteSpace(asset.LocalPath) || !File.Exists(asset.LocalPath)) throw new FileNotFoundException("Deliverable asset file is missing.", asset.LocalPath);
            var info = new FileInfo(asset.LocalPath); if (info.Length <= 0 || info.Length != asset.FileLength) throw new InvalidDataException("Deliverable asset length has changed: " + asset.Id);
            if (!string.Equals(Hash(asset.LocalPath), asset.ContentHashSha256, StringComparison.OrdinalIgnoreCase)) throw new InvalidDataException("Deliverable asset hash has changed: " + asset.Id);
        }
    ...
    public sealed class CaptureIntegrityService : DeliverableIntegrityService { }
}
