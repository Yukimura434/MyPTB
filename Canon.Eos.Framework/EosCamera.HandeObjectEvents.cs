...
namespace Canon.Eos.Framework
{
    partial class EosCamera
    {
        ...
        {
            // In Camera+Host mode Canon emits DirItemCreated for the card copy and
            // DirItemRequestTransfer for the host copy. Only the latter is a
            // downloadable object. Treating both as PictureTaken queues the same
            // image twice and can leave the real transfer request blocked.
        ...
        private void OnObjectEventDirItemRequestTransfer(IntPtr sender)
        {
            ...
            if (this.PictureTaken == null)
            {
                Edsdk.EdsDownloadCancel(sender);
                Edsdk.EdsRelease(sender);
                return;
            }
            try
