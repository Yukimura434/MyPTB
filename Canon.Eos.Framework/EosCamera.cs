...
namespace Canon.Eos.Framework
{
    public sealed partial class EosCamera : EosObject
    {
        ...
        private void ChangePicturesSaveLocation(SaveLocation saveLocation)
        {
            ...
            if(!this.IsLegacy)
            {
                // EdsSetCapacity is the required follow-up when SaveTo includes Host.
                // It does not require CameraState_UILock. Newer EOS bodies (including
                // the EOS RP) can return DEVICE_BUSY for that status command while the
                // session itself is valid, which used to abort initialization and keep
                // Live View from ever starting.
...
//                var v = this.GetPropertyStruct<Edsdk.EdsTime>(Edsdk.PropID_DateTime, Edsdk.EdsDataType.Time);
        ...
        {
            // A USB removal invalidates the native handle before the asynchronous
            // shutdown callback reaches the application. Stopping EVF is therefore
            // best-effort: INVALID_HANDLE/DEVICE_NOT_FOUND means it is already off.
            this._pauseLiveViewRequested = true;
            this._cancelLiveViewRequested = true;
            if (Handle == IntPtr.Zero)
                return;
            Edsdk.EdsSetPropertyData(Handle, Edsdk.PropID_Evf_OutputDevice, 0,
                Marshal.SizeOf(typeof(int)), (int)EosLiveViewDevice.None);
        }
