...
namespace Canon.Eos.Framework
{
    public abstract class EosObject : EosDisposable
    {
        ...
        /// <summary>
        /// Reads optional/transient EDSDK metadata without throwing. EVF metadata is
        /// allowed to disappear while the camera switches between live view and the
        /// shutter, so an error result is a normal state transition rather than an
        /// exceptional application failure.
        /// </summary>
        protected bool TryGetPropertyStruct<T>(uint propertyId, out T value) where T : struct
        {
            value = default(T);
            ...
            try
            {
                var result = Edsdk.EdsGetPropertyData(Handle, propertyId, 0, size, pointer);
